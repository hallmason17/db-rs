...
impl Table {
    ...
    fn find_page_with_free_space(&mut self, bp: &mut BufferPool) -> anyhow::Result<u64> {
        let fsm_num = self.current_fsm_idx;
        tracing::info!("Getting FSM {:?}", fsm_num);
        let fsm = bp.get_page(PageId {
            file_id: self.id,
            page_num: fsm_num,
        })?;
        let fsm = fsm.as_fsm()?;
        let ffp = fsm.find_first_free_page(self.last_known_free_page);
        self.last_known_free_page = ffp;
        Ok(ffp)
    }

    fn set_fsm_page_full(&self, bmgr: &mut BufferPool, page_num: u64) -> anyhow::Result<()> {
        let mut fsm = bmgr.get_page(PageId {
            file_id: self.id,
    ...
    fn is_fsm_full(&self, bmgr: &mut BufferPool) -> anyhow::Result<bool> {
        let fsm = bmgr.get_page(PageId {
            file_id: self.id,
            page_num: self.current_fsm_idx,
        })?;
    ...
    fn handle_full_page(&mut self, page_num: u64, bp: &mut BufferPool) -> anyhow::Result<()> {
        self.set_fsm_page_full(bp, page_num)?;
        let is_fsm_full = self.is_fsm_full(bp)?;

        if is_fsm_full {
            tracing::warn!("FSM full!");
            let new_fsm_num = {
                let mut new_fsm_page = bp.create_page(self.id, PageKind::FreeSpaceMap)?;
                let new_fsm = new_fsm_page.as_fsm_mut()?;
                new_fsm.header().page_id()
            };

            let mut old_fsm = bp.get_page(PageId {
                file_id: self.id,
                page_num: self.current_fsm_idx,
            })?;
    ...
    pub fn insert_record(
        ...
        let mut free_page = {
            let mut pnum = self.find_page_with_free_space(bp)?;
            if pnum == u64::MAX {
                let new_page = bp.create_page(self.id, PageKind::Heap)?;
        ...
        loop {
            match self.try_insert_into_page(free_page, record, bp)? {
                Some(rid) => return Ok(rid),
                None => {
                    self.handle_full_page(free_page, bp)?;
                    free_page = self.find_page_with_free_space(bp)?;
                }
            }
        }
    }
